...
class SensitivityAnalysis:
    def __init__(self, data,df_data, target_index=None, sensitive_name='sex', sensitive_index=8, log_data=True,
        ...
        self.data_writer2.writerow(
            ['ID', 'Feature', 'Category','swap_proportion','Acc', 'Acc_after', 'distortion_multiple','distortion_feature', 'Distortion_hellinger','Distortion_wasserstein','cramers_v_distortion', 'total_variation_distortion', 'Distortion_kl', 'Distortion_js', 'hellinger_div','wasserstein_div', 'cramers_v_div','total_variation_div', 'KL_div', 'JS_Div', 'Casuality', 'Importance','worse_case_unfair', 'epsilon_unfair',
    ...
    def compute_metrics(self, perc_parity, x_test_altered, random_index_, category_indices, y_predicted_before, y_predicted_after, column_id, p_pred,
        ...
        for category in category_list:
            ...
            kl_mean, js_mean, hellinger_mean, wasserstein_mean, cramers_v_mean, total_variation_mean, worse_case_unfairness_mean, sigma_unfairness_mean = 0, 0, 0, 0, 0, 0, 0, 0
            if len(a_before_sub) > 1 and len(b_before_sub) > 0:
                ...
                worse_case_unfairness_pos, sigma_unfairness_pos = DistanceMeasure2.fair_degree(a_before_sub, a_after_sub, fair_error)
                worse_case_unfairness_neg, sigma_unfairness_neg = DistanceMeasure2.fair_degree(b_before_sub, b_after_sub,
                                                                                       fair_error)
                worse_case_unfairness_mean = np.mean([worse_case_unfairness_pos, worse_case_unfairness_neg])
                sigma_unfairness_mean = np.mean([sigma_unfairness_pos, sigma_unfairness_neg])
                ...
                #cramers_v_pos = DistanceMeasure.cramers_v(a_before_sub, a_after_sub)
                total_variation_distance_pos = DistanceMeasure2.total_variation_distance(a_before_sub, a_after_sub)
                worse_case_unfairness_pos, sigma_unfairness_pos = DistanceMeasure2.fair_degree(a_before_sub,
                                                                                               a_after_sub, fair_error)
                kl_mean = kl_pos
                js_mean = js_pos
                worse_case_unfairness_mean = worse_case_unfairness_pos
                sigma_unfairness_mean = sigma_unfairness_pos
                ...
                #cramers_v_neg = DistanceMeasure.cramers_v(b_before_sub, b_after_sub)
                total_variation_distance_neg = DistanceMeasure2.total_variation_distance(b_before_sub, b_after_sub)
                worse_case_unfairness_neg, sigma_unfairness_neg = DistanceMeasure2.fair_degree(b_before_sub,
                                                                                               b_after_sub,
                                                                                               fair_error)
                ...
                worse_case_unfairness_mean = worse_case_unfairness_neg
                sigma_unfairness_mean = sigma_unfairness_neg
            ...
            if len(category_target_original)>0:
                ...
                data_category[category]['worse_case_unfairness'] = worse_case_unfairness_mean
                data_category[category]['sigma_unfairness'] = sigma_unfairness_mean
    ...
    def compute_metrics_continous(self, perc_parity, x_test_altered, random_index_, category_indices, y_predicted_before, y_predicted_after, column_id, p_pred,
                                  q_pred, distortion_multiple, distortion_feature, hellinger_distortion, wasserstein_distance_distortion,cramers_v_distortion,total_variation_distortion, kl_div_distortion, js_div_distortion, runkl=False):
        # for column_id in range(self.x_train.shape[1]):
        #    if is_categorical(self.x_test,column_id):
        p_pred = abs(p_pred)
            ...
            for swap_proportion_ in swap_proportion:
                ...
                for column_id in range(self.x_train.shape[1]):
                    ...
                    if is_categorical(self.x_test, column_id):
                        ...
                        data_category_after = self.compute_metrics(alpha, x_test_altered, random_index_, category_indices, list_before, list_after, column_id, p_pred,
                        ...
                        random_index_, x_test_altered = alter_feature_value_continous_3(self.x_test, self.y_test, self.determine_range(column_id),
                                                                       column_id, swap_proportion=swap_proportion_)
                        # q_pred = NNmodel.predict(tf.convert_to_tensor(x_test_altered, dtype=tf.float32))
                        q_pred = NNmodel.predict(x_test_altered)
                        list_after = []
                        ...
                        #                                                                          x_test_altered)
                        #print('cramers_v: ', cramers_v)
                        data_category_after = self.compute_metrics_continous(alpha, x_test_altered,random_index_, category_indices, list_before, list_after,
                    ...
                    for key, val in data_category_after.items():
                        ...
                        self.data_writer2.writerow(
                            ['F_{}'.format(column_id), self.colums_list[column_id], key,swap_proportion_, data_category_after[key]['ACC'],
                             ...
                             data_category_after[key]['Importance'], data_category_after[key]['worse_case_unfairness'],
                             data_category_after[key]['sigma_unfairness'], data_category_after[key]['SP'],
                    ...
                    if len(a_before) > 1 and len(b_before) > 0:
                        ...
                        ##todo: Fair degree
                        worse_case_unfairness_pos, sigma_unfairness_pos = DistanceMeasure2.fair_degree(a_before, a_after,
                                                                                                       fair_error)
                        worse_case_unfairness_neg, sigma_unfairness_neg = DistanceMeasure2.fair_degree(b_before, b_after,
                                                                                                       fair_error)
                        worse_case_unfairness_mean = np.mean([worse_case_unfairness_pos, worse_case_unfairness_neg])
                        sigma_unfairness_mean = np.mean([sigma_unfairness_pos, sigma_unfairness_neg])
                        ...
                        total_variation_distance_pos = DistanceMeasure2.total_variation_distance(a_before, a_after)
                        ##todo: Fair degree
                        worse_case_unfairness_pos, sigma_unfairness_pos = DistanceMeasure2.fair_degree(a_before,
                                                                                                       a_after,
                                                                                                       fair_error)
                        ...
                        cramers_v_neg = DistanceMeasure.cramers_v(b_before, b_after)
                        total_variation_distance_neg = DistanceMeasure2.total_variation_distance(b_before, b_after)
                        worse_case_unfairness_neg, sigma_unfairness_neg = DistanceMeasure2.fair_degree(b_before,
                                                                                                       b_after,
                                                                                                       fair_error)
                    ...
                    if self.log_data:
                        ...
                        self.data_writer.writerow(
                            ['F_{}'.format(column_id), self.colums_list[column_id],swap_proportion_, acc_category, acc_category_after, distortion_multiple, distortion_feature,
                             ...
                             kl_mean, js_mean, casuality, f_importance, worse_case_unfairness_mean, sigma_unfairness_mean, SP, SP_after, TPR, TPR_after, FPR,
                             FPR_after])
...
if __name__ == '__main__':
    ...
    data_name = 'adult-{}_'.format(perc_parity)  # _35_threshold
    ...
    sensitivityAnalysis.fit_nn(alpha=perc_parity)
