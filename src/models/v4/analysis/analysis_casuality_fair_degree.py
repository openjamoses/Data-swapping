...
#todo: global
group_data = df_global.groupby(['ID', 'Feature','swap_proportion'])['Casuality', 'Importance', 'worse_case_unfair', 'epsilon_unfair'].mean().reset_index()
#print(len(group_data))
#print(group_data.head())
#group_data.to_csv(path + 'logging3/processed/KL_Divergence_local_adult_processed_global.csv', index=False)
data_file = open(path+path_output +'/processed'+'/Casuality_local_{}_processed_global_casuality.csv'.format(data_name), mode='w', newline='',
                 encoding='utf-8')
...
Importance = group_data.Importance.values.tolist()
worse_case_unfair = group_data.worse_case_unfair.values.tolist()
epsilon_unfair = group_data.epsilon_unfair.values.tolist()
for i in range(len(Feature)):
    ...
    data_writer.writerow(
        [ID[i], Feature[i], swap_proportion[i], '{}%'.format(swap_prop), 'Propensity\n score(+)',
         Casuality[i]])
    data_writer.writerow(
        [ID[i], Feature[i], swap_proportion[i], '{}%'.format(swap_prop), 'Worse case\n unfair degree(+)',
         worse_case_unfair[i]])
    data_writer.writerow(
        [ID[i], Feature[i], swap_proportion[i], '{}%'.format(swap_prop), 'Epsilon\n unfair degree(+)',
         epsilon_unfair[i]])
data_file.close()
...
'Distortion_js', 'hellinger_div', 'wasserstein_div','cramers_v_div','JS_Div', 'Casuality', 'Importance', 'worse_case_unfair', 'epsilon_unfair'].mean().reset_index()

data_file = open(path+path_output +'/processed'+'/Casuality_local_{}_processed_local_difference.csv'.format(data_name), mode='w', newline='',
                 encoding='utf-8')
data_writer = csv.writer(data_file)
...
for i in range(len(Feature)):
    if Feature[i] in data_dict_overal.keys():
        if swap_proportion[i] in data_dict_overal[Feature[i]].keys():
            data_dict_overal[Feature[i]][swap_proportion[i]][Category[i]] = [Casuality[i], Importance[i], worse_case_unfair[i], epsilon_unfair[i]]
        else:
            data_dict_overal[Feature[i]][swap_proportion[i]] = {}
            data_dict_overal[Feature[i]][swap_proportion[i]][Category[i]] = [Casuality[i],
                                                                             Importance[i], worse_case_unfair[i], epsilon_unfair[i]]
...
for key, val in data_dict_overal.items():
    ...
    for key2, val2 in val.items():
        ...
        if val_protected != None and val_non_protected != None:
            ...
            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Worse case\n unfair degree(+)', val_non_protected[2] - val_protected[2],
                 np.std([val_protected[2], val_non_protected[2]]),
                 '{}\n[{}]'.format(key, data_protected_attrib.get(key)), data_non_protected_attrib.get(key)])
            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Epsilon\n unfair degree(+)', val_non_protected[3] - val_protected[3],
                 np.std([val_protected[3], val_non_protected[3]]),
                 '{}\n[{}]'.format(key, data_protected_attrib.get(key)), data_non_protected_attrib.get(key)])
            ...
            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Worse case\n unfair degree(+)', val_non_protected[2],
                 np.std([0, val_non_protected[2]]), '{}\n[{}]'.format(key, data_protected_attrib.get(key)),
                 data_non_protected_attrib.get(key)])

            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Epsilon\n unfair degree(+)', val_non_protected[3],
                 np.std([0, val_non_protected[3]]), '{}\n[{}]'.format(key, data_protected_attrib.get(key)),
                 data_non_protected_attrib.get(key)])
            ...
            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Worse case\n unfair degree(+)', 0 - val_protected[2],
                 np.std([val_protected[2], 0]), '{}\n[{}]'.format(key, data_protected_attrib.get(key)),
                 data_non_protected_attrib.get(key)])

            data_writer.writerow(
                [key, key2, '{}%'.format(swap_prop), 'Epsilon\n unfair degree(+)', 0 - val_protected[3],
                 np.std([val_protected[3], 0]), '{}\n[{}]'.format(key, data_protected_attrib.get(key)),
                 data_non_protected_attrib.get(key)])
data_file.close()
#print(group_data)
