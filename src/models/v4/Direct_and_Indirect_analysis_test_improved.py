...
class SensitivityAnalysis:
    def __init__(self, data,df_data, target_index=None, sensitive_name='sex', sensitive_index=8, log_data=True,
        ...
        self.log_path_global = log_path + 'Divergence_improved_{}.csv'.format(data_name)
        ...
        self.log_path_local = log_path + 'Divergence_improved_2_local_{}.csv'.format(data_name)
    ...
    def compute_metrics_continous_direct(self, perc_parity, x_test_altered, random_index_, category_indices, y_predicted_before, y_predicted_after, column_id, p_pred,
        ...
        for key, val in category_data.items():
            ...
            if len(val)>0 and len(category_target_predicted_before)> 0:
                ...
                data_category[category]['JS_Divergence'] = js_mean
        return data_category
    def compute_(self, data_category, perc_parity, x_test_altered, random_index_, category_indices, y_predicted_before, y_predicted_after, p_pred, q_pred, column_id, column_id2, val, category_1, distortion_multiple, distortion_feature, columns_names):
        category_target_original = []
        category_target_predicted_before = []
        category_target_predicted_after_mediator = []
        category_target_predicted_after_feature = []
        category_target_predicted_after_both = []

        category_target_predicted_before_indirect = []
        category_target_original_indirect = []
        category_target_predicted_before_indirect = []
        category_target_predicted_after_both_indirect = []
        a_before_sub_indirect, b_before_sub_indirect = [], []
        a_after_sub_both_indirect, b_after_sub_both_indirect = [], []
        ...
        category_2 = columns_names[column_id2]
        TP = 0
        FP = 0
        TN = 0
        FN = 0
        ...
        FN_after = 0
        a_before_sub, b_before_sub = [], []
        a_after_sub_both, b_after_sub_both = [], []
        a_after_sub_feature, b_after_sub_feature = [], []
        a_after_sub_mediator, b_after_sub_mediator = [], []
        ...
        (x_test_altered_both, x_test_altered_feature_only, x_test_altered_mediator_only) = x_test_altered
        (list_after_both, list_after_f_only, list_after_m_only) = y_predicted_after
        (q_pred_both, q_pred_f_only, q_pred_m_only) = q_pred
        q_pred_both, q_pred_f_only, q_pred_m_only = abs(q_pred_both), abs(q_pred_f_only), abs(q_pred_m_only)
        for i in range(len(self.x_test)):
            cont_swap_direct = True
            cont_swap_indirect = True
            if i in random_index_ and perc_parity != None:
                #if perc_parity != None:
                #distortion_value_both = DistanceMeasure2._distance_single(self.x_test[i], x_test_altered_both[i],
                #                                                     category_indices)
                distortion_value_indirect = DistanceMeasure2._distance_single(x_test_altered_feature_only[i], x_test_altered_both[i],
                                                                     category_indices)
                distortion_value_direct = DistanceMeasure2._distance_single(x_test_altered_mediator_only[i], x_test_altered_both[i],
                                                                     category_indices)
                if distortion_value_direct > perc_parity:
                    cont_swap_direct = False
                # x_test_altered_row = x_test_altered[i]
                # distortion_value = DistanceMeasure2._distance_single(self.x_test[i], x_test_altered[i])
            if (self.x_test[i][column_id] in val) and cont_swap_direct:
                category_target_original.append(self.y_test[i])
                category_target_predicted_before.append(y_predicted_before[i])
                category_target_predicted_after_both.append(list_after_both[i])
                category_target_predicted_after_mediator.append(list_after_m_only[i])
                # print(y_predicted[i], len(self.y_test[i]), self.y_test[i], self.y_test)
                if y_predicted_before[i] == 1 and self.y_test[i] == 1:
                    TP += 1
                if y_predicted_before[i] == 1 and self.y_test[i] == 0:
                ...
                if y_predicted_before[i] == 0 and self.y_test[i] == 1:
                    FN += 1
                if list_after_both[i] == 1 and self.y_test[i] == 1:
                    TP_after += 1
                if list_after_both[i] == 1 and self.y_test[i] == 0:
                    FP_after += 1
                if list_after_both[i] == 0 and self.y_test[i] == 0:
                    TN_after += 1
                if list_after_both[i] == 0 and self.y_test[i] == 1:
                    FN_after += 1
                # if runkl:
                a_before_sub.append(p_pred[i][0])
                b_before_sub.append(p_pred[i][1])
                # if q_pred != None:
                a_after_sub_both.append(q_pred_both[i][0])
                b_after_sub_both.append(q_pred_both[i][1])

                a_after_sub_mediator.append(q_pred_m_only[i][0])
                b_after_sub_mediator.append(q_pred_m_only[i][1])

                x_test_modified.append(self.x_test[i])
            if (self.x_test[i][column_id] in val) and cont_swap_indirect:
                ...
                category_target_predicted_after_both_indirect.append(list_after_both[i])
                category_target_predicted_after_feature.append(list_after_f_only[i])
                if y_predicted_before[i] == 1 and self.y_test[i] == 1:
                    TP_indirect += 1
                if y_predicted_before[i] == 1 and self.y_test[i] == 0:
                    FP_indirect += 1
                ...
                # if q_pred != None:
                a_after_sub_both_indirect.append(q_pred_both[i][0])
                b_after_sub_both_indirect.append(q_pred_both[i][1])

                a_after_sub_feature.append(q_pred_f_only[i][0])
                b_after_sub_feature.append(q_pred_f_only[i][1])

        category_target_original = np.array(category_target_original)
        category_target_predicted_before = np.array(category_target_predicted_before)
        category_target_predicted_after_both = np.array(category_target_predicted_after_both)
        #category_target_predicted_after_feature = np.array(category_target_predicted_after_feature)
        category_target_predicted_after_mediator = np.array(category_target_predicted_after_mediator)
        acc_category = np.sum(category_target_original == category_target_predicted_before)
        TPR = calculate_TPR(TP, FP, TN, FN)
        FPR = calculate_FPR(TP, FP, TN, FN)
        SP = calculate_SP(TP, FP, TN, FN)

        acc_category_after_direct = np.sum(category_target_predicted_after_mediator == category_target_predicted_after_both)
        # if str(acc_category_after) == 'nan':
        f_importance_direct = np.sum(category_target_predicted_after_mediator != category_target_predicted_after_both)
        TPR_after = calculate_TPR(TP_after, FP_after, TN_after, FN_after)
        FPR_after = calculate_FPR(TP_after, FP_after, TN_after, FN_after)
        SP_after = calculate_SP(TP_after, FP_after, TN_after, FN_after)
        ...
        proportion_direct = np.sum(
            category_target_predicted_before != category_target_predicted_after_both)  # calculate_proportion(TP, FP, TN, FN)
        kl_pos = None
        kl_neg = None
        ...
        if len(val) > 0 and len(category_target_predicted_before) > 0:
            ...
            if len(a_before_sub) > 1 and len(b_before_sub) > 0:
               ...
               #print('checking mean 1 starts..')
                ...
                kl_pos, js_pos = DistanceMeasure2.js_divergence(a_after_sub_mediator, a_after_sub_both)
                kl_neg, js_neg = DistanceMeasure2.js_divergence(b_after_sub_mediator, b_after_sub_both)
                #print('checking kl_pos error ends..')
                # todo: Hellinger Distance
                #print('checking Hellinger error starts..')
                #print('Hellinger distance starts ----')
                hellinger_pos = DistanceMeasure2.hellinger_continous_1d(a_after_sub_mediator, a_after_sub_both)
                hellinger_neg = DistanceMeasure2.hellinger_continous_1d(b_after_sub_mediator, b_after_sub_both)
                #print('checking Hellinger error ends..')
                #print('Hellinger distance ends ----')
                # todo: wasserstein Distance
                #print('checking wasserstein error starts..')
                wasserstein_distance_pos = DistanceMeasure2.wasserstein_distance(a_after_sub_mediator, a_after_sub_both)
                wasserstein_distance_neg = DistanceMeasure2.wasserstein_distance(b_after_sub_mediator, b_after_sub_both)
                ...
                total_variation_distance_pos = DistanceMeasure2.total_variation_distance(a_after_sub_mediator, a_after_sub_both)
                total_variation_distance_neg = DistanceMeasure2.total_variation_distance(b_after_sub_mediator, b_after_sub_both)
                #print('checking total_variation_distance_pos error ends..')


                #print('checking effect_distance_pos error starts..')
                effect_distance_pos, effect_distance_contrained_pos = DistanceMeasure2.effect_distance(a_after_sub_mediator, a_after_sub_both,
                                                                                                       fair_error)
                effect_distance_neg, effect_distance_contrained_neg = DistanceMeasure2.effect_distance(a_after_sub_mediator, a_after_sub_both,
                                                                                                       fair_error)
                effect_distance_mean = np.mean([effect_distance_pos, effect_distance_neg])
                effect_distance_contrained_mean = np.mean(
                    [effect_distance_contrained_pos, effect_distance_contrained_neg])
                ...
                # import matplotlib.pyplot as plt
                # fig, ax = plt.subplots(1, 1)
                effect_distance_neg, effect_distance_contrained_neg = DistanceMeasure2.effect_distance(b_after_sub_mediator, b_after_sub_both,
                                                                                                       fair_error)
            ...
            cramers_v_mean = DistanceMeasure.cramers_v(category_target_predicted_after_mediator,
                                                       category_target_predicted_after_both)
            ...
            data_category[category_1][category_2][IMPACT_TYPE]['ACC_after'] = round(acc_category_after_direct * 100 / len(category_target_original),
                                                         3)
            ...
            data_category[category_1][category_2][IMPACT_TYPE]['proportion'] = round(proportion_direct * 100 / len(category_target_original), 3)
            data_category[category_1][category_2][IMPACT_TYPE]['Importance'] = round(f_importance_direct * 100 / len(category_target_original), 3)
        ...
        if len(val) > 0 and len(category_target_predicted_before_indirect) > 0:
            category_target_original_indirect = np.array(category_target_original_indirect)
            category_target_predicted_before_indirect = np.array(category_target_predicted_before_indirect)
            category_target_predicted_after_both_indirect = np.array(category_target_predicted_after_both_indirect)
            category_target_predicted_after_feature = np.array(category_target_predicted_after_feature)
            acc_category = np.sum(category_target_original_indirect == category_target_predicted_before_indirect)
            TPR = calculate_TPR(TP_indirect, FP_indirect, TN_indirect, FN_indirect)
            FPR = calculate_FPR(TP_indirect, FP_indirect, TN_indirect, FN_indirect)
            SP = calculate_SP(TP_indirect, FP_indirect, TN_indirect, FN_indirect)

            acc_category_after_indirect = np.sum(
                category_target_predicted_after_feature == category_target_predicted_after_both_indirect)
            # if str(acc_category_after) == 'nan':
            f_importance_direct = np.sum(
                category_target_predicted_after_feature != category_target_predicted_after_both_indirect)
            TPR_after = calculate_TPR(TP_after_indirect, FP_after_indirect, TN_after_indirect, FN_after_indirect)
            FPR_after = calculate_FPR(TP_after_indirect, FP_after_indirect, TN_after_indirect, FN_after_indirect)
            SP_after = calculate_SP(TP_after_indirect, FP_after_indirect, TN_after_indirect, FN_after_indirect)

            # if len(category_target_original) == 0:
            #    print('f_importance: ', category, f_importance, category_target_original, category_target_predicted_after)

            proportion_indirect = np.sum(
                category_target_predicted_before_indirect != category_target_predicted_after_both_indirect)

            #f_importance = np.sum(category_target_predicted_after_feature != category_target_predicted_after_both_indirect)
            kl_pos = None
            kl_neg = None
            ...
            # print('checking starts..')
            kl_mean, js_mean, hellinger_mean, wasserstein_mean, cramers_v_mean, total_variation_mean, effect_distance_mean, effect_distance_contrained_mean = 0, 0, 0, 0, 0, 0, 0, 0
            if len(a_before_sub_indirect) > 1 and len(b_before_sub) > 0:
                # todo: KL Divergence
                # print('checking mean 1 starts..')
                # print('checking kl_pos error starts..')
                kl_pos, js_pos = DistanceMeasure2.js_divergence(a_after_sub_feature, a_after_sub_both_indirect)
                kl_neg, js_neg = DistanceMeasure2.js_divergence(b_after_sub_feature, b_after_sub_both_indirect)
                # print('checking kl_pos error ends..')
                # todo: Hellinger Distance
                # print('checking Hellinger error starts..')
                # print('Hellinger distance starts ----')
                hellinger_pos = DistanceMeasure2.hellinger_continous_1d(a_after_sub_feature, a_after_sub_both_indirect)
                hellinger_neg = DistanceMeasure2.hellinger_continous_1d(b_after_sub_feature, b_after_sub_both_indirect)
                # print('checking Hellinger error ends..')
                # print('Hellinger distance ends ----')
                # todo: wasserstein Distance
                # print('checking wasserstein error starts..')
                wasserstein_distance_pos = DistanceMeasure2.wasserstein_distance(a_after_sub_feature, a_after_sub_both_indirect)
                wasserstein_distance_neg = DistanceMeasure2.wasserstein_distance(b_after_sub_feature, b_after_sub_both_indirect)
                # print('checking wasserstein error ends..')
                # todo: cramers_v Distance
                # cramers_v_pos = DistanceMeasure.cramers_v(a_before_sub, a_after_sub)
                # cramers_v_neg = DistanceMeasure.cramers_v(b_before_sub, b_after_sub)
                # todo: total variation distance
                # print('checking total_variation_distance_pos error starts..')
                total_variation_distance_pos = DistanceMeasure2.total_variation_distance(a_after_sub_feature, a_after_sub_both_indirect)
                total_variation_distance_neg = DistanceMeasure2.total_variation_distance(b_after_sub_feature, b_after_sub_both_indirect)
                # print('checking total_variation_distance_pos error ends..')

                # print('checking effect_distance_pos error starts..')
                effect_distance_pos, effect_distance_contrained_pos = DistanceMeasure2.effect_distance(a_after_sub_feature, a_after_sub_both_indirect,
                                                                                                       fair_error)
                effect_distance_neg, effect_distance_contrained_neg = DistanceMeasure2.effect_distance(b_after_sub_feature, b_after_sub_both_indirect,
                                                                                                       fair_error)
                effect_distance_mean = np.mean([effect_distance_pos, effect_distance_neg])
                effect_distance_contrained_mean = np.mean(
                    [effect_distance_contrained_pos, effect_distance_contrained_neg])
                ...
                total_variation_mean = np.mean([total_variation_distance_pos, total_variation_distance_neg])
                # print('checking mean 1 end..')
            elif len(a_before_sub_indirect) > 1:
                # print('checking mean 2 starts..')
                kl_pos, js_pos = DistanceMeasure2.js_divergence(a_after_sub_feature, a_after_sub_both_indirect)
                hellinger_pos = DistanceMeasure2.hellinger_continous_1d(a_after_sub_feature, a_after_sub_both_indirect)
                wasserstein_distance_pos = DistanceMeasure2.wasserstein_distance(a_after_sub_feature, a_after_sub_both_indirect)
                ...
                total_variation_mean = total_variation_distance_pos
                # print('checking mean 2 ends..')
            elif len(b_before_sub_indirect) > 1:
                # print('checking mean 3 starts..')
                kl_neg, js_neg = DistanceMeasure2.js_divergence(b_after_sub_feature, b_after_sub_both_indirect)
                hellinger_neg = DistanceMeasure2.hellinger_continous_1d(b_after_sub_feature, b_after_sub_both_indirect)
                wasserstein_distance_neg = DistanceMeasure2.wasserstein_distance(b_after_sub_feature, b_after_sub_both_indirect)
            ...
            # print('checking cramers_v_mean error ends..')
            # print('checking 1 starts..')
            cramers_v_mean = DistanceMeasure.cramers_v(category_target_predicted_after_feature,
                                                       category_target_predicted_after_both_indirect)
            ...
            if not category_2 in data_category[category_1].keys():
                data_category[category_1][category_2] = {}
            data_category[category_1][category_2][IMPACT_TYPE] = {}
            data_category[category_1][category_2][IMPACT_TYPE]['ACC'] = acc_category
            data_category[category_1][category_2][IMPACT_TYPE]['TPR'] = TPR
            data_category[category_1][category_2][IMPACT_TYPE]['FPR'] = FPR
            data_category[category_1][category_2][IMPACT_TYPE]['distortion_multiple'] = distortion_multiple
            data_category[category_1][category_2][IMPACT_TYPE]['distortion_feature'] = distortion_feature
            ...
            data_category[category_1][category_2][IMPACT_TYPE]['SP'] = SP
            data_category[category_1][category_2][IMPACT_TYPE]['ACC_after'] = round(acc_category_after_indirect * 100 / len(category_target_original_indirect), 3)
            data_category[category_1][category_2][IMPACT_TYPE]['TPR_after'] = TPR_after
            data_category[category_1][category_2][IMPACT_TYPE]['FPR_after'] = FPR_after
            ...
            data_category[category_1][category_2][IMPACT_TYPE]['proportion'] = round(
                proportion_indirect * 100 / len(category_target_original_indirect), 3)
            data_category[category_1][category_2][IMPACT_TYPE]['Importance'] = round(
                f_importance_direct * 100 / len(category_target_original_indirect), 3)
            data_category[category_1][category_2][IMPACT_TYPE]['hellinger_div'] = hellinger_mean
            data_category[category_1][category_2][IMPACT_TYPE]['wasserstein_distance_div'] = wasserstein_mean
            data_category[category_1][category_2][IMPACT_TYPE]['cramers_v_div'] = cramers_v_mean
            data_category[category_1][category_2][IMPACT_TYPE]['total_variation_div'] = total_variation_mean
    ...
    def compute_metrics_indirect(self, perc_parity, x_test_altered, random_index_, category_indices, y_predicted_before, y_predicted_after, column_id,column_id2, p_pred,
                                  q_pred,distortion_multiple,distortion_feature, columns_names=[], runkl=False):
        # for column_id in range(self.x_train.shape[1]):
        #    if is_categorical(self.x_test,column_id):
        p_pred = abs(p_pred)
        #q_pred = abs(q_pred)
        ...
        if is_categorical(self.x_test, column_id):
            ...
            for cat_ in category_list:
                ...
                self.compute_(data_category, perc_parity, x_test_altered, random_index_, category_indices,
                              ...
                              category_1, distortion_multiple, distortion_feature, columns_names)
                ...
                for column_id in range(self.x_train.shape[1]):
                    ...
                    for column_id2 in range(self.x_train.shape[1]):
                        ...
                        if column_id != column_id2 and feature_list.index(feat1) <= feature_list.index(feat2):
                            random_index_, x_test_altered_feature_only, x_test_altered_mediator_only, x_test_altered_both = alter_2_columns(self.x_test, self.y_test, self.get_posible_values_2_columns([column_id, column_id2]),
                            ...
                            a_after_both, b_after_both = [], []
                            a_after_f_only, b_after_f_only = [], []
                            a_after_m_only, b_after_m_only = [], []
                            #if is_categorical(self.x_test, column_id):
                            # q_pred = NNmodel.predict(tf.convert_to_tensor(x_test_altered, dtype=tf.float32))
                            q_pred_both = NNmodel.predict(x_test_altered_both)
                            q_pred_f_only = NNmodel.predict(x_test_altered_feature_only)
                            q_pred_m_only = NNmodel.predict(x_test_altered_mediator_only)

                            #print('q_pred: ', q_pred)
                            list_after_both, list_after_f_only, list_after_m_only = [],[],[]
                            for i_ in range(len(q_pred_both)):
                                #pred_ = abs(pred_)
                                a_after_both.append(q_pred_both[i_][0])
                                a_after_both.append(q_pred_both[i_][1])

                                a_after_f_only.append(q_pred_f_only[i_][0])
                                b_after_f_only.append(q_pred_f_only[i_][1])

                                a_after_m_only.append(q_pred_m_only[i_][0])
                                b_after_m_only.append(q_pred_m_only[i_][1])

                                arg_max_both = q_pred_both[i_].argmax(axis=-1)
                                arg_max_f = q_pred_f_only[i_].argmax(axis=-1)
                                arg_max_m = q_pred_m_only[i_].argmax(axis=-1)
                                #print('q_pred: ', pred_, pred_2, abs(pred_), abs(pred_[0]), abs(pred_[1]), arg_max)
                                list_after_both.append(arg_max_both)
                                list_after_f_only.append(arg_max_f)
                                list_after_m_only.append(arg_max_m)
                            distortion_multiple, distortion_feature = 0,0 #hellinger_distortion, hellinger_distortion

                            data_category_2_columns_after = self.compute_metrics_indirect(alpha, (x_test_altered_both, x_test_altered_feature_only, x_test_altered_mediator_only),
                                                                                                  random_index_,
                                                                                                  category_indices,
                                                                                                  list_before,
                                                                                                  (list_after_both, list_after_f_only, list_after_m_only),
                                                                                                  column_id,column_id2, p_pred,
                                                                                                  (q_pred_both, q_pred_f_only, q_pred_m_only),
                                                                                                  distortion_multiple,
                                                                                                  distortion_feature, columns_names=columns_names,
                                                                                                  runkl=True)
