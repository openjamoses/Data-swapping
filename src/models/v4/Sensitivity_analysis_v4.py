...
from src.models.v4.sensitivity_utils_4 import *
...
class SensitivityAnalysis:
    def __init__(self, data, target_index=None, sensitive_name='sex', sensitive_index=8, log_data=True,
        ...
        self.data_writer.writerow(
            ['ID', 'Feature','Category', 'swap_proportion', 'Acc', 'Acc_after', 'Distortion_hellinger','Distortion_wasserstein','cramers_v_distortion', 'total_variation_distortion',  'Distortion_kl', 'Distortion_js', 'hellinger_div','wasserstein_div','cramers_v_div', 'total_variation_div', 'KL_div', 'JS_Div', 'Casuality', 'Importance', 'SP', 'SP_after',
        ...
        self.data_writer2.writerow(
            ['ID', 'Feature', 'Category','swap_proportion','Acc', 'Acc_after', 'Distortion_hellinger','Distortion_wasserstein','cramers_v_distortion', 'total_variation_distortion', 'Distortion_kl', 'Distortion_js', 'hellinger_div','wasserstein_div', 'cramers_v_div','total_variation_div', 'KL_div', 'JS_Div', 'Casuality', 'Importance',
    ...
    def compute_metrics(self, random_indices, y_predicted_before, y_predicted_after, column_id, p_pred,
                        q_pred,hellinger_distortion, wasserstein_distance_distortion,cramers_v_distortion, total_variation_distortion, kl_div_distortion, js_div_distortion, runkl=False):
        ...
        for category in category_list:
            ...
            for i in range(len(random_indices)):
                if self.x_test[random_indices[i]][column_id] == category:
                    ...
                    if y_predicted_before[i] == 1 and self.y_test[random_indices[i]] == 1:
                        TP += 1
                    if y_predicted_before[i] == 1 and self.y_test[random_indices[i]] == 0:
                        FP += 1
                    if y_predicted_before[i] == 0 and self.y_test[random_indices[i]] == 0:
                        TN += 1
                    if y_predicted_before[i] == 0 and self.y_test[random_indices[i]] == 1:
                        FN += 1
            ...
            if str(impont) == 'nan' or len(category_target_original) == 0:
                print('nan found! ',category, category_list, column_id, random_indices, category_target_original, )
            ...
            if len(category_target_original)>0:
                print('total_variation_mean: ', total_variation_mean)
    ...
    def compute_metrics_continous(self, random_indices, y_predicted_before, y_predicted_after, column_id, p_pred,
                                  q_pred, hellinger_distortion, wasserstein_distance_distortion,cramers_v_distortion,total_variation_distortion, kl_div_distortion, js_div_distortion, runkl=False):
        # for column_id in range(self.x_train.shape[1]):
        #    if is_categorical(self.x_test,column_id):
        p_pred = abs(p_pred)
        ...
        for key, val in category_data.items():
            ...
            if len(val)>0 and len(category_target_predicted_before)> 0:
                ...
                if len(a_before_sub) >1 and len(b_before_sub) > 0:
                    ...
                    cramers_v_mean = cramers_v_neg
                    total_variation_mean = total_variation_distance_neg
                print('total_variation_mean: ', total_variation_mean, a_before_sub, b_before_sub)
    ...
    def fit_nn(self, iterations=10, swap_proportion=[0.01, 0.05, 0.1,  0.2,  0.4, 0.6,  0.8, 0.95]):
        ...
        for swap_proportion_ in swap_proportion:
            for iter in range(iterations):
                ...
                p_pred = NNmodel.predict(x_sampled_k)
                a_before, b_before = [], []
                ...
                for column_id in range(self.x_train.shape[1]):
                    ...
                    a_after, b_after = [], []
                    if is_categorical(self.x_test, column_id):
                        random_index_, x_test_altered = alter_feature_values_categorical(x_sampled_k, random_index,
                        ...
                        data_category_after = self.compute_metrics(random_index_, list_before, list_after, column_id, p_pred,
                                                                   q_pred, hellinger_distortion, wasserstein_distance_distortion,cramers_v_distortion, total_variation_distortion, kl_div_distortion, js_div_distortion, runkl=True)

                    else:
                        random_index_, x_test_altered = alter_feature_value_continous(x_sampled_k, random_index, self.determine_range(column_id),
                                                                       column_id, swap_proportion=swap_proportion_)
                        # q_pred = NNmodel.predict(tf.convert_to_tensor(x_test_altered, dtype=tf.float32))
                        q_pred = NNmodel.predict(x_test_altered)
                        list_after = []
                        ...
                        total_variation_distortion = DistanceMeasure2.total_variation_distance_2d(x_sampled_k,
                                                                                                  x_test_altered)
                        #print('cramers_v: ', cramers_v)
                        data_category_after = self.compute_metrics_continous(random_index_, list_before, list_after,
                                                                             column_id, p_pred, q_pred, hellinger_distortion, wasserstein_distance_distortion,cramers_v_distortion, total_variation_distortion,kl_div_distortion, js_div_distortion, runkl=True)
                    for key, val in data_category_after.items():
                        ...
                        self.data_writer2.writerow(
                            ['F_{}'.format(column_id), self.colums_list[column_id], key,swap_proportion_, data_category_after[key]['ACC'],
                             data_category_after[key]['ACC_after'], data_category_after[key]['hellinger_distortion'],
                    ...
                    if self.log_data:
                        ...
                        for i in range(len(random_index)):
                            ...
                            if list_before[i] == 1 and self.y_test[random_index[i]] == 1:
                                TP += 1
                            if list_before[i] == 1 and self.y_test[random_index[i]] == 0:
                                FP += 1
                            if list_before[i] == 0 and self.y_test[random_index[i]] == 0:
                                TN += 1
                            if list_before[i] == 0 and self.y_test[random_index[i]] == 1:
                                FN += 1
                        ...
                        y_test_ = [self.y_test[random_index[i]] for i in range(len(random_index))]
                        ...
                        self.data_writer.writerow(
                            ['F_{}'.format(column_id), self.colums_list[column_id],swap_proportion_, acc_category, acc_category_after,
