...
class GeneticAlgorithsm:
    def __init__(self, model, _features_posible_value, target_index, sensitive_indices, sensitive_index_chosen=0, max_test=1000000, data_name=''):
        #self.data = data
        self.model = model
        ...
        self.test_suite_file = open(self.path+'TestSuit_{}.txt'.format(data_name), 'w+')
    ...
    def _crossover(self, data_dict):
        ...
        if len(data_new) > 1:
            ...
            for index in range(len(data_1)):
                ...
                range_list = [i for i in range(1, len(data_1[0])) if i != self.target_index]
                #print(type(range_list), range_list, range(1, len(data_1[0])), len(data_1[0]))
                #print(type(range_list), range_list, range(1, len(data_1[0])), len(data_1[0]))
...
class TestCaseGeneration:
    def __init__(self, train, test, sensitive_list, sensitive_indices, target_index, data_name=''):
        #self.data = data
    ...
    def get_categorical_features_posible_value(self, feature_index):
        print('column data: ', set(self.train[0:, feature_index]))
        data_folded = {}
        for val in list(set(self.train[0:, feature_index])):
            data_folded[val] = [val]
        return data_folded #list(set(self.data[0:, feature_index]))
    def determine_range(self, feature_index, k_folds=3):
        data_range = self.train[0:, feature_index]
    ...
    def model_train(self, data_name):
        ...
        algo = GeneticAlgorithsm(NNmodel,self.get_categorical_features_posible_value(column_id),target_index,self.sensitive_indices,0,data_name=data_name)
...
if __name__ == '__main__':
    ...
    path_ = '../dataset/experiments/data/'
    load_data = LoadTrainTest(path_)
    data_name = 'bank'
    train, test, target_index = load_data.load_bank(data_name)
    ...
    # df_adult = loadData.load_student_data('Student.csv')
    sensitive_list = load_data.sensitive_list
    sensitive_indices = load_data.sensitive_indices
    colums_list = train.columns.tolist()
    target_name = load_data.target_name
    target_index = load_data.target_index

    testGeneration = TestCaseGeneration(train.to_numpy(), test.to_numpy(),sensitive_list,sensitive_indices,target_index)
    testGeneration.model_train(data_name=data_name)
