...
class SensitivityAnalysis:
    def __init__(self, data, target_index=None,sensitive_name='sex', sensitive_index=8, log_data=True, log_path='../dataset/logging/', data_name='', colums_list=[], threshold=None ):
        ...
        if self.log_data:
            ...
            self.data_writer.writerow(['ID','Feature','Acc','Acc_after', 'KL_Pos','KL_Neg','Casuality', 'Importance','SP', 'SP_after', 'TPR', 'TPR_after', 'FPR', 'FPR_after'])
            ...
            self.data_writer2.writerow(['ID','Feature','Category', 'Acc','Acc_after', 'KL_Pos', 'KL_Neg','Casuality', 'Importance', 'SP', 'SP_after', 'TPR', 'TPR_after', 'FPR', 'FPR_after'])
    ...
    def determine_range(self, feature_index, k_folds=3):
        ...
        #return folded_data
    def compute_metrics(self, random_indices,y_predicted_before, y_predicted_after, column_id, p_pred=None, q_pred=None, runkl=False):
    ...
    def compute_metrics_continous(self,random_indices, y_predicted_before, y_predicted_after,  column_id, p_pred=None, q_pred=None, runkl=False):
        #for column_id in range(self.x_train.shape[1]):
        #    if is_categorical(self.x_test,column_id):
        data_category = {}
    ...
    def fit_nn(self, iterations=10):
        ...
        for iter in range(iterations):
            ...
            for column_id in range(self.x_train.shape[1]):
                ...
                a_after, b_after = [], []
                if is_categorical(self.x_test,column_id):
                    x_test_altered = alter_feature_values_categorical(x_sampled_k, self.get_categorical_features(column_id),
                                                                      column_id)
                    #q_pred = NNmodel.predict(tf.convert_to_tensor(x_test_altered, dtype=tf.float32))
                    q_pred = NNmodel.predict(x_test_altered)
                    list_after = []
                    ...
                    data_category_after = self.compute_metrics(random_index, list_before, list_after, column_id, p_pred, q_pred, runkl=True)
                    ...
                    for pred_ in q_pred:
                        ...
                        arg_max = pred_.argmax(axis=-1)
                        list_after.append(arg_max)
                    data_category_after = self.compute_metrics_continous(random_index, list_before, list_after, column_id, p_pred,q_pred, runkl=True)
                for key, val in data_category_after.items():
                    self.data_writer2.writerow(
                        ['F_{}'.format(column_id), self.colums_list[column_id],key, data_category_after[key]['ACC'], data_category_after[key]['ACC_after'], data_category_after[key]['KL_Pos'], data_category_after[key]['KL_Neg'], data_category_after[key]['proportion'], data_category_after[key]['Importance'], data_category_after[key]['SP'], data_category_after[key]['SP_after'], data_category_after[key]['TPR'], data_category_after[key]['TPR_after'], data_category_after[key]['FPR'], data_category_after[key]['FPR_after']])
                ...
                if self.log_data:
                    ...
                    self.data_writer.writerow(
                        ['F_{}'.format(column_id), self.colums_list[column_id], acc_category, acc_category_after, kl_div_positive, kl_div_negative, casuality,f_importance, SP, SP_after, TPR, TPR_after, FPR,
                         FPR_after])
