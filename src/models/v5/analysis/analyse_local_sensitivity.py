...
#data_name = 'Student'
#data_name = 'compas'
path_output = 'logging4/{}/'.format(data_name)
df_global = pd.read_csv(path + 'logging4/Divergence_local_{}.csv'.format(data_name))
...
'Distortion_js', 'hellinger_div', 'wasserstein_div','cramers_v_div', 'total_variation_div','JS_Div', 'Casuality', 'Importance'].mean().reset_index()
print(group_data)
data_file = open(path+path_output +'/processed'+ '/Divergence_local_{}_processed_local_difference.csv'.format(data_name), mode='w', newline='',
                 encoding='utf-8')
data_writer = csv.writer(data_file)
